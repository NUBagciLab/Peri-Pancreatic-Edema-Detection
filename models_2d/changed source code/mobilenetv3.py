...
# --------------------------------------------MobileNetV3 for CT pancreas training----------------------------------
class Mobilenet_v3_pancreas(nn.Module):
    def __init__(
        self,
        inverted_residual_setting: List[InvertedResidualConfig],
        last_channel: int,
...
@register_model()
@handle_legacy_interface(weights=("pretrained", MobileNet_V3_Small_Weights.IMAGENET1K_V1))
def mobilenet_v3_pancreas(
    *, weights:Optional[MobileNet_V3_Small_Weights] = None,arch:str, progress: bool = True, **kwargs: Any
) -> Mobilenet_v3_pancreas:
    weights = MobileNet_V3_Small_Weights.verify(weights)
    if arch == 'mobilenet_v3_small':
        ...
        inverted_residual_setting, last_channel = _mobilenet_v3_conf(arch, **kwargs)
    return _mobilenet_v3_pancreas(inverted_residual_setting, last_channel, weights,progress, **kwargs)


def _mobilenet_v3_pancreas(
    inverted_residual_setting: List[InvertedResidualConfig],
    last_channel: int,
    weights: Optional[WeightsEnum],
    progress: bool,
    **kwargs: Any,
    ...
    model = Mobilenet_v3_pancreas(inverted_residual_setting, last_channel, **kwargs)
