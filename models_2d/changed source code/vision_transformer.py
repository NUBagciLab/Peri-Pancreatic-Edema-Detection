...
# -----------------------------vision transfomer only for pancreas-------------------

class VisionTransformer_panceras(nn.Module):
    """Vision Transformer as per https://arxiv.org/abs/2010.11929."""
...
def _vision_transformer_pancreas(
    patch_size: int,
    num_layers: int,
    num_heads: int,
    hidden_dim: int,
    ...
    **kwargs: Any,
    ...
    model = VisionTransformer_panceras(
        image_size=image_size,
        patch_size=patch_size,
        num_layers=num_layers,
        num_heads=num_heads,
...
@register_model()
@handle_legacy_interface(weights=("pretrained", ViT_B_16_Weights.IMAGENET1K_V1))
def vit_b_16_pancreas(*, weights: Optional[ViT_B_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer_panceras:
    """
    Constructs a vit_b_16 architecture from
    `An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale <https://arxiv.org/abs/2010.11929>`_.
    ...
    return _vision_transformer_pancreas(
        patch_size=16,
        num_layers=12,
        num_heads=12,
        hidden_dim=768,
